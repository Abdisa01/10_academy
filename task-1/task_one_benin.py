...
# Load the dataset
df_benin = pd.read_csv('C:\\Users\\Lalaa\\Documents\\GitHub\\10_academy\\Data\\benin-malanville.csv')
...
df_benin['comulative_GHI']=df_benin['GHI'].cumsum()
df_benin['comulative_DNI']=df_benin['DNI'].cumsum()
plt.figure(figsize=(14, 8))
plt.plot(df_benin.index, df_benin['Cumulative_GHI'], label='Cumulative GHI', color='orange')
plt.plot(df_benin.index, df_benin['Cumulative_DNI'], label='Cumulative DNI', color='blue')
...
print("Available columns in data:", df_benin.columns.tolist())
#relevant column to correlete 
rel_column=['GHI', 'DNI', 'DHI', 'TModA', 'TModB', 'WS', 'WSgust', 'WD']
data= df_benin[rel_column].dropna() #to drop nan values
